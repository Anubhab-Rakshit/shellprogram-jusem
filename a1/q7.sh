...
echo -n "Enter another word to replace it :- "
read replace_str

total_count=$(grep -o "$search_str" "$filename" | wc -l | tr -d ' ')
exact_count=$(grep -o -w "$search_str" "$filename" | wc -l | tr -d ' ')
...
if [ "$total_count" -gt "$exact_count" ]
then
    echo "Partial matches do exist."
fi

if grep -i -q "$search_str" "$filename"
then
    echo "Match exists if case sensitivity is ignored."
fi
